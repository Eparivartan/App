...
import 'package:careercoach/Learn%20Assist/Guest%20Lecture%20play.dart';
...
class GuestLecture extends StatefulWidget {
  const GuestLecture({Key? key}) : super(key: key);

  @override
  State<GuestLecture> createState() => _GuestLectureState();
}

class _GuestLectureState extends State<GuestLecture> {
  ...
  Future getTrainingVideosDetails(from) async{
    ...
    final response = await http.get(Uri.parse('${Config.baseURL}listguestlectures/$from/5'));
    // final serviceResponse = await http.get(Uri.parse(''));
    if(response.statusCode == 200){
      var jsonData = jsonDecode(response.body)["guestlectures"];
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: PreferredSize(
        ...
        child: App_Bar_widget(title: 'Guest Lectures',),
      ),
      body: SafeArea(
        child: Padding(
            ...
            children: [
              Container(
                child: RichText(text: TextSpan(
                    children: [
                      TextSpan(text: 'Guest Lectures contains ',
                        style: TextStyle(
                      ...
                      TextSpan(text: 'all the collated videos from '
                          'e-content and other important university '
                          'syllabi …',
                        style: TextStyle(
                            fontSize: Config.font_size_12.sp,
                ...
                child: ListView.builder(
                    ...
                    itemBuilder: (context, index)
                    {
                      return Padding(
                        ...
                        child: InkWell(
                          onTap: () {
                            Navigator.of(context).push(MaterialPageRoute(builder: (context)=> GuestLecturePlay(index1: index)));
                          },
                          child: Container(
